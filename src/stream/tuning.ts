...
// Entry bound, evicted oldest-first alongside the byte bound. 512 was reached
// around 20 turns (storeSize 514) while the 128 MiB byte cap still had headroom;
// the miss dropped the checkpoint and the rebuild overflowed Cursor (~1.5 MiB /
// ~388k tokens). 4096 keeps Map overhead bounded without punching holes in long
// agent sessions. The journal persists the live store under the same entry cap.
export const MAX_ACTIVE_BLOB_ENTRIES = 4096;
